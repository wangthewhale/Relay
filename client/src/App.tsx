...
import type {
  AccessRequirement, AgentRun, ApprovalRequest, CollaborationSnapshot, CompilerReceipt, Conflict, ConnectorDescriptor, CreateMissionInput, ExecutionTask, MissionDetail, MissionMember, MissionSummary, Outcome, PlanVersion, PublicMissionReport, SourceInput,
} from "@shared/domain";
  ...
  return <div className={`flow-canvas-layout ${inspectorOpen ? "" : "inspector-collapsed"} ${mobileCanvasOpen ? "mobile-canvas-visible" : ""}`}>
    <section className="mobile-mission-journey" aria-label={tr("Mobile mission execution story", "手機版 Mission 執行流程")}>
      ...
      <div className="mobile-now-stack">
        ...
        <button type="button" className="mobile-open-canvas" onClick={() => setMobileCanvasOpen(true)}><span><Maximize2 size={20}/><p><b>{tr("See the whole mission map", "查看完整 Mission 全局")}</b><small>{tr("Open the zoomable canvas only when you need the big picture", "需要看全局時，再打開可縮放畫布")}</small></p></span><ArrowRight size={19}/></button>
    ...
    <section className="flow-canvas" aria-label={tr("Mission execution canvas", "Mission 執行 Canvas")}>
      <div className="mobile-canvas-bar"><button type="button" onClick={() => setMobileCanvasOpen(false)}><X size={19}/>{tr("Back to Now", "返回現在")}</button><span>{tr("Pinch to zoom · tap a block to add context", "雙指縮放 · 點區塊加入內容")}</span></div>
