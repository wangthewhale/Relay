      ...
      <main>
        <section className="hero">
          <div className="hero-copy">
            <div className="eyebrow"><span className="pulse-dot" /> {tr("HUMANS + AI · ASYNC PROJECT EXECUTION", "人類 + AI 的非同步專案執行室")}</div>
            <h1>{locale === "zh-TW" ? <><span className="hero-line">人類只管目標、</span><span className="hero-line">授權與確認；</span><span className="hero-line">Relay 讓 AI 接手團隊待辦，</span><span className="hero-line hero-accent-line">一路執行到完成。</span></> : <><span className="hero-line">Humans set goals,</span><span className="hero-line">approve and confirm.</span><span className="hero-line">Relay AI does the work</span><span className="hero-line hero-accent-line">all the way to done.</span></>}</h1>
            <h2>{locale === "zh-TW" ? <>財務、社群、CEO、工程師與實習生都在同一個 Project Room 說明各自目標。<b>Relay 讓 1–3 個 AI Agent 統整待辦、向正確角色取得授權、執行並通知全員。</b></> : <>Finance, social, the CEO, engineering and interns state their goals in one Project Room. <b>Relay gives 1–3 AI agents the work, routes exact approvals to the right roles, executes and keeps everyone informed.</b></>}</h2>
            <p>{tr("Start with cross-functional campaigns and product launches. Replace status meetings, follow-up messages and waiting with one shared plan, precise approvals and an auditable execution feed.", "先從跨部門 Campaign 與產品 Launch 開始：用一份共用計畫、精確授權與可稽核執行動態，取代進度會議、反覆追問與等待。")}</p>
