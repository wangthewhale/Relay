...
function InviteTeammatesDialog({ mission, open, onClose, onCopy }: { mission: MissionDetail; open: boolean; onClose: () => void; onCopy: () => void }) {
  if (!open) return null;
  const roomUrl = `${window.location.origin}/missions/${mission.id}?view=room`;
  return <div className="invite-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="invite-dialog" role="dialog" aria-modal="true" aria-labelledby="invite-dialog-title">
      ...
      <p className="invite-dialog-intro">{tr("There is no setup form. Send this link to the people who should see the evidence or make a decision.", "不用設定帳號或角色。把這個連結傳給需要看證據或做決定的同事即可。")}</p>
      ...
      <p className="invite-privacy"><ShieldCheck size={14} /> {tr("Public MVP: anyone with this link can open the mission. Do not share confidential information yet.", "公開 MVP：取得連結的人都能開啟這份任務；現階段請勿放入機密資訊。")}</p>
    </section>
  </div>;
}
...
function MissionPage() {
  ...
  const [mission, setMission] = useState<MissionDetail>(); const [error, setError] = useState(""); const [busy, setBusy] = useState(""); const [notice, setNotice] = useState(""); const [lastSyncedAt, setLastSyncedAt] = useState(""); const [inviteOpen, setInviteOpen] = useState(false);
  ...
  const copyRoomLink = async () => {
    try {
      await navigator.clipboard.writeText(`${window.location.origin}/missions/${id}?view=room`);
      setNotice(tr("Invite link copied. Send it to teammates in Slack or email.", "邀請連結已複製，現在可以貼到 Slack 或 Email 傳給同事。"));
    } catch {
