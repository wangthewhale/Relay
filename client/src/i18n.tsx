...
const labels: Record<string, string> = {
  ...
  minutes_ready: "紀錄完成",
  waiting: "等待中",
  waiting_on_human: "等待人類",
  decision_needed: "需要決定",
  verified: "已驗證",
  read: "讀取",
  write: "寫入",
  connected: "已授權",
...
const domainText: Record<string, string> = {
  ...
  "Refund action is simultaneously required and forbidden": "退款同時被要求執行，也被政策禁止",
  "One source commits the team to a refund while another source forbids issuing it under the current payment process.": "一個來源已承諾退款，另一個來源卻禁止在目前付款流程下執行退款。",
  "Continuing both paths can create a duplicate refund, an unresolved chargeback, or a misleading customer promise.": "同時繼續兩條路徑可能造成重複退款、未結拒付，或對客戶做出錯誤承諾。",
  "Pause both financial actions, verify the active payment state, then let the Finance owner select exactly one settlement path.": "先暫停兩項財務操作、確認目前付款狀態，再由財務負責人只選擇一條結算路徑。",
  "Close the chargeback path before issuing a newly approved refund.": "先結束拒付流程，再執行重新核准的退款。",
  "Keep the chargeback path and send a human-reviewed correction to the customer.": "保留拒付流程，並向客戶寄出經人工審查的更正說明。",
  "The approved delivery scope has been superseded": "已核准的交付範圍已被新指令取代",
  "An approved deliverable set and a later replacement instruction cannot both remain the active scope.": "已核准的交付項目與後續替換指令，不能同時作為目前有效範圍。",
  "Agents may deliver obsolete work, omit the replacement deliverables, or create avoidable rework.": "Agent 可能交付過期內容、漏掉替代項目，或造成可以避免的返工。",
  "Confirm the replacement instruction as the current scope, invalidate affected drafts, and compile a new plan version.": "確認替換指令為目前有效範圍，使受影響草稿失效，並編譯新的計畫版本。",
  "Keep the approved scope and ask the requester to withdraw the replacement instruction.": "保留原核准範圍，並請提出者撤回替換指令。",
  "Pause delivery and request one exact scope signed off by both owners.": "暫停交付，請雙方負責人共同確認一份精確範圍。",
